from machine import Pin
from TRSensor import TRSensor
from Motor import MotorControl
from ws2812 import LEDControl
import time

#Beispiel Code um den PicoGo einer schwarzen Linie auf hellem Grund folgen zu lassen
#Zu Begin dreht sich der PicoGo, um sich zu kalibrieren
#Hat dieselbe Funktionalität wie Line-Tracking.py mit folgenden Änderungen:
#Roboter stoppt wenn ein Hindernis von den IR-Sensoren vorne erkannt wird
#Dabei piept der Roboter und fährt erst weiter, wenn das Hindernis entfernt wurde
#Die LEDs sind aktiv: zu Beginn leuchten sie in verschiedenen Farben. Beim Fahren durchlaufen sie einen Regenbogen



M = MotorControl()
Buzzer = Pin(4, Pin.OUT)
DSR = Pin(2, Pin.IN)
DSL = Pin(3, Pin.IN)

strip = LEDControl()
strip.pixels_set(0, strip.RED)
strip.pixels_set(1, strip.GREEN)
strip.pixels_set(2, strip.BLUE)
strip.pixels_set(3, strip.YELLOW)
strip.pixels_show()  
time.sleep(2)

TRS=TRSensor()
for i in range(100):
    if(i<25 or i>= 75):
        M.setMotor(30,-30)
    else:
        M.setMotor(-30,30)
    TRS.calibrate()
print("\ncalibrate done\r\n")
print(TRS.calibratedMin)
print(TRS.calibratedMax)
print("\ncalibrate done\r\n")
maximum = 30 #die Geschwindikeit mit der der Linie gefolgt werden soll
integral = 0
last_proportional = 0
j=0

while True:
    position,Sensors = TRS.readLine()
    DR_status = DSR.value()
    DL_status = DSL.value()
    
    if((Sensors[0] + Sensors[1] + Sensors[2]+ Sensors[3]+ Sensors[4]) > 4000):
        Buzzer.value(0) #Buzzer.value(0) heißt kein Piepton
        M.setMotor(0,0)
    elif((DL_status == 0) or (DR_status == 0)):
        Buzzer.value(1) #Buzzer.value(1) aktiviert Piepton
        M.setMotor(0,0)
    else:
        Buzzer.value(0)
        # "proportional" ist 0, wenn der Roboter auf der Linie ist
        proportional = position - 2000

        derivative = proportional - last_proportional

        last_proportional = proportional
        
        power_difference = proportional/30  + derivative*2;  

        if (power_difference > maximum):
            power_difference = maximum
        if (power_difference < - maximum):
            power_difference = - maximum

        if (power_difference < 0):
            M.setMotor(maximum + power_difference, maximum)
        else:
            M.setMotor(maximum, maximum - power_difference)

    #LED Steuerung
    for i in range(strip.num):
        strip.pixels_set(i, strip.wheel(((i * 256 // strip.num) + j) & 255))
    strip.pixels_show()
    j += 1
    if(j > 256): 
        j = 0
